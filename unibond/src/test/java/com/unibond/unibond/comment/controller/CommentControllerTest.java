        ...
        this.mockMvc.perform(
                ...
                .andDo(document("get-child-comment-with-page",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("result.pageInfo").type(OBJECT).description("자식 댓글 페이징 정보"),
                                fieldWithPath("result.pageInfo.numberOfElements").type(NUMBER).description("자식 댓글 페이징 정보: 현재 페이지의 원소 개수"),
                                fieldWithPath("result.pageInfo.lastPage").type(BOOLEAN).description("자식 댓글 페이징 정보: 마지막 페이지인지의 여부"),
                                fieldWithPath("result.pageInfo.totalPages").type(NUMBER).description("자식 댓글 페이징 정보: 총 페이지 수"),
                                fieldWithPath("result.pageInfo.totalElements").type(NUMBER).description("자식 댓글 페이징 정보: 총 검색 결과 개수"),
                                fieldWithPath("result.pageInfo.size").type(NUMBER).description("자식 댓글 페이징 정보: 현재 페이지의 크기 [default: 30] - size는 parameter를 통해 전송하지 않는 것을 추천드립니다."),
                                fieldWithPath("result.childCommentList").type(ARRAY).description("자식 댓글 리스트").optional(),
